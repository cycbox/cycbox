...
-- Parses PMS9103M particulate matter sensor data and sends alerts via
-- Discord webhook, ntfy push notification, and SMTP email when PM2.5
-- levels exceed a high threshold or recover below a low threshold.
--
-- Frame format:
--   Prefix: 0x42 0x4D ("BM")
...
-- ntfy configuration
local NTFY_TOPIC = "cycbox_air_quality"  -- Replace with your unique topic name
...
-- "normal": PM2.5 is below low threshold
-- "alert":  PM2.5 is above high threshold
local alert_state = "normal"

-- ========== Notification Helpers ==========

local function send_high_alert(pm2_5_value)
    local msg = string.format(
        "PM2.5 ALERT: %d μg/m³ (threshold: %d μg/m³). Air quality is unhealthy!",
        pm2_5_value, PM25_HIGH_THRESHOLD
    )
...
local function send_recovery_alert(pm2_5_value)
    local msg = string.format(
        "PM2.5 RECOVERY: %d μg/m³ (threshold: %d μg/m³). Air quality restored to normal.",
        pm2_5_value, PM25_LOW_THRESHOLD
    )
    ...
    ntfy_send_async({
        topic    = NTFY_TOPIC,
        message  = msg,
        title    = "PM2.5 Restored to Normal",
        priority = "default",
        tags     = "white_check_mark,herb",
    })
    ...
    smtp_send_async({
        ...
        from     = SMTP_CONFIG.from,
        to       = SMTP_CONFIG.to,
        subject  = string.format("PM2.5 Restored: %d μg/m³", pm2_5_value),
        text     = msg,
    })
...
function on_receive()
    ...
    -- Check PM2.5 thresholds and send notifications
